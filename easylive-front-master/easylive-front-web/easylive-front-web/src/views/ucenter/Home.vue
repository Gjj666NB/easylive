    ...
    <div class="video-data-list">
      ...
      >
        <div class="video-data-title">
          <div :class="['name iconfont', item.icon]">总{{ item.name }}</div>
          <div class="pre-count">昨日新增：{{ item.preCount }}</div>
        </div>
...
const loadWeekData = async () => {
  ...
  console.log(dateArray, dataCountArray);
  const option = {
    title: {
      text: `近7天新增${currentDataPart.value.name}`,
    },
    tooltip: {
      trigger: "axis",
    },
